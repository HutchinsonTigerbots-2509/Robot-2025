...
public class pathPlannerDrive extends SubsystemBase {
  ...
  vision sVision;
   ...
   static Pose2d startPose2d;
         ...
         public void periodic() {
           ...
           //SmartDashboard.getData(AutoBuilder.getCurrentPose().);
                   ...
                   if (DriverStation.isDisabled() && SmartDashboard.getBoolean("ApplyStart", false)) {
                     ApplyStart();
                     eSwerveEstimator.resetPose(startPose2d);
                     setGyro(eSwerveEstimator.getEstimatedPosition().getRotation().getDegrees());
                   }
     ...
     public void setEstimator(Pose2d pos) {
        eSwerveEstimator.resetPosition(pos.getRotation(), getModulePositions(), pos);
     }
    ...
    */
    public void namedCommands() {
        ...
        // Actions
            ...
            NamedCommands.registerCommand("Intake Out Slow " + n, new RunCommand(() -> sIntake.intakeSet(-.5)).withTimeout(n).andThen(new InstantCommand(() -> sIntake.intakeStop())));
        ...
        // Preset Poses
            ...
            NamedCommands.registerCommand("MoveVisionFeeder " + (n), fixMoveAuto(0, (n + 3.0)));
            NamedCommands.registerCommand("MoveVisionLeft " + (n), fixMoveAuto(-.4, (n + 3.0)));
            NamedCommands.registerCommand("MoveVisionRight " + (n), fixMoveAuto(.4, (n + 3.0)));
    ...
    public void buildAutoChooser() {
      autoSelect.setDefaultOption("M1V", AutoBuilder.buildAuto("M1V"));
    ...
    // public command autoMaker(String name) {
    //   PathPlannerPath pathAuto = new PathPlannerAuto;
    //   AutoBuilder.followPath(pathAuto);
    //   return pathAuto;
    // }
    ...
    public SequentialCommandGroup fixMoveAuto(double kCam, double timeout) {
      double desiredPos = kCam;
      double driveTime = 1;
      double fixerTime = timeout - driveTime;

      return new SequentialCommandGroup(
        new RunCommand(() -> {driveChassis(new ChassisSpeeds(0, sVision.getMoveVision(desiredPos), 0));}).withTimeout(fixerTime),
        new RunCommand(() -> {driveChassis(new ChassisSpeeds(-.5, 0, 0));}).withTimeout(driveTime)
      );
    }
